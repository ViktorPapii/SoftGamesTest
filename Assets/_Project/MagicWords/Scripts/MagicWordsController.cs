...
{
    /// <summary>
    /// Loads the payload, stages the cast and plays the script one line at a time. Advance,
    /// ToggleAutoPlay and ToggleHistory are inspector-wired UnityEvent targets; Begin and Replay
    /// are handed to the completion popup as its Retry.
    /// </summary>
    public class MagicWordsController : MonoBehaviour
    {
        ...
        [Tooltip("The loading state only. Finishing and failing both go to the shared popup.")]
        ...
        [Tooltip("Fetch and play as soon as the scene is up. Off waits for Begin.")]
        ...
        /// <summary>
        /// Both endings raise the same popup, and each hands it the retry that fits: a script that
        /// played to the end replays from line one, while one that never arrived is fetched again.
        /// </summary>
        private void Finish()
        {
            _state = State.Finished;
            GameManager.Instance.Completion.Show("That's the last line.", Replay);
        }
        ...
        private void Fail(string message)
        {
            _state = State.Failed;
            statusPanel.SetActive(false);
            GameManager.Instance.Completion.Show(message, Begin);
        }
