...
{
    /// <summary>
    /// Rebuilds the colour cycle from the palette asset: one state per entry, named after it,
    /// chained on the "Next" trigger, every state playing the same fade clip. Run it after adding,
    /// removing or renaming an entry — that name is the only link between the two, and
    /// FlamePaletteGraphTests fails when they drift.
    ///
    /// The palette asset is the authority and is only read. Existing assets are rewritten in place
    /// rather than replaced: a fresh asset would take a new GUID and quietly empty the slot in the
