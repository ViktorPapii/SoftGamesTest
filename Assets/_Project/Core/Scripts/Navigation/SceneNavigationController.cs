...
namespace SoftGames.Core
{
    ...
    public class SceneNavigationController : MonoBehaviour, IGameNavigation
    {
        ...
        private async Awaitable LoadAsync(string sceneName)
        {
            ...
            {
                // IsOnMenu answers differently from here, and the cover is still opaque: scene
                // chrome takes its place now, so the reveal shows it already settled rather than
                // popping it in afterwards.
                StateChanged?.Invoke();
